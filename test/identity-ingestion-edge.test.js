...
test("runs the resumable username ledger from a bounded Cloudflare schedule", () => {
  ...
  assert.match(config, /"USERNAME_FRAGMENT_PREFIX_DEPTH": "8"/);
...
test("exposes authenticated knowledge and full-cycle diagnostics", () => {
  ...
  assert.match(worker, /knowledgeBatchSize\(env, "USERNAME_KNOWLEDGE_FAST_BATCH_SIZE", 12, 12\)/);
  assert.match(worker, /knowledgeBatchSize\(env, "DNS_KNOWLEDGE_FAST_BATCH_SIZE", 2, 2\)/);
  assert.match(worker, /knowledgeBatchSize\(env, "USERNAME_KNOWLEDGE_FULL_BATCH_SIZE", 3, 3\)/);
  assert.match(worker, /knowledgeBatchSize\(env, "DNS_KNOWLEDGE_FULL_BATCH_SIZE", 1, 1\)/);
  assert.match(config, /"USERNAME_KNOWLEDGE_FAST_BATCH_SIZE": "12"/);
  assert.match(config, /"USERNAME_KNOWLEDGE_FULL_BATCH_SIZE": "3"/);
