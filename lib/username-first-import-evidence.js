...
function createUsernameFirstImportEvidence(options = {}) {
  ...
  const maxKnowledgeAssets = Math.max(0, Math.min(maxAssets, Number(options.maxKnowledgeAssets ?? process.env.USERNAME_FIRST_IMPORT_KNOWLEDGE_LIMIT ?? 12)));
  const fullKnowledgeAssets = Math.max(0, Math.min(maxKnowledgeAssets, Number(options.fullKnowledgeAssets ?? process.env.USERNAME_FIRST_IMPORT_FULL_KNOWLEDGE_LIMIT ?? 12)));
  ...
  async function enrich(assets = []) {
    ...
    const results = await Promise.all(eligible.map(async (target, index) => {
      const shouldResolveKnowledge = Boolean(knowledgeResolver && index < maxKnowledgeAssets);
      try {
        const [historyResult, knowledgeResult] = await Promise.allSettled([
          ...
          shouldResolveKnowledge
            ? knowledgeResolver(target.record.normalizedName, { fast: index >= fullKnowledgeAssets, maxAttempts: 1 })
            : Promise.resolve({}),
        ]);
        ...
        return { inspected: { ...target, reportedSales: (history.events || []).length, verifiedSales: accepted }, verifiedEvents };
      } catch (error) {
        logger.warn?.(`[username-first-import] ${target.record.normalizedName} deferred: ${String(error.message || error).slice(0, 160)}`);
        return { inspected: { ...target, reportedSales: 0, verifiedSales: 0, error: String(error.message || error) }, verifiedEvents: [] };
      }
    }));
    const inspected = results.map((result) => result.inspected);
    const verifiedEvents = results.flatMap((result) => result.verifiedEvents);
