...
function indicativeBaselineResult(target, baseline, marketSignals) {
  ...
  const observedLowGram = finiteOr(baseline.rangeLowGram, 0);
  const observedHighGram = finiteOr(baseline.rangeHighGram, 0);
  // A no-sale DNS fallback is only meaningful when its cohort is reasonably
  // concentrated. A huge interquantile spread means the route mixes scarce
  // premium names with ordinary labels; its median cannot price an unrelated
  // imported domain safely.
  const lowRatio = observedLowGram > 0 ? midpointGram / observedLowGram : Infinity;
  const highRatio = observedHighGram > 0 ? observedHighGram / midpointGram : Infinity;
  if (lowRatio > 20 || highRatio > 100) return null;
