...
function paymentMatch(transaction, expectedAmount, maximumFeeRatio = 0.08) {
  ...
  if (candidates[0]) return { ...candidates[0], kind: "net-proceeds" };

  // Fragment has used a 5% marketplace fee plus a fixed conversion fee for
  // newly collectible usernames. Older sales used a 1 GRAM fixed fee, while
  // current documentation describes a 5 GRAM fee. Match those explicit fee
  // schedules with only a small allowance for transaction gas.
  const fixedFees = [1n * NANO_PER_GRAM, 5n * NANO_PER_GRAM];
  const percentageFees = [0, 0.05];
  const gasTolerance = 100_000_000n;
  const scheduled = [];
  for (const value of values) {
    for (const percentageFee of percentageFees) {
      for (const fixedFee of fixedFees) {
        const afterPercentage = BigInt(Math.round(Number(expectedAmount) * (1 - percentageFee)));
        const scheduledAmount = afterPercentage - fixedFee;
        if (scheduledAmount <= 0n || value > scheduledAmount) continue;
        const gasDifference = scheduledAmount - value;
        if (gasDifference > gasTolerance) continue;
        scheduled.push({
          amount: value,
          deviation: Number(expectedAmount - value) / Number(expectedAmount),
          feeRatio: percentageFee,
          fixedFeeGram: Number(fixedFee) / Number(NANO_PER_GRAM),
          gasDifferenceGram: Number(gasDifference) / Number(NANO_PER_GRAM),
          kind: "fragment-fee-schedule",
        });
      }
    }
  }
  scheduled.sort((left, right) => left.gasDifferenceGram - right.gasDifferenceGram);
