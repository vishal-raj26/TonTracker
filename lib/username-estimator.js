...
function estimateTelegramUsernameValue(targetInput, events = [], options = {}) {
  ...
  if (lastSaleAnchor) {
    ...
    // evidence. Comparables may scale it upward, but may not dilute it below
    // the latest sale after applying the measured segment market movement.
    p50 = Math.max(p50, lastSaleAnchor.marketAdjustedUsd);
