...
function createUsernameRuntime(options = {}) {
      ...
      const events = (payload.records || []).flatMap((row) => {
        try {
          return [{
            ...
            classification: classifyTelegramUsername(row.normalized_name),
          }];
        } catch { return []; }
      });
  ...
  function merge(asset, valuation) {
    ...
    const lowConfidence = String(valuation.confidenceBand || "").toLowerCase() === "low";
    return { ...asset,
      floorTon: 0, floorUsd: eligible ? estimateUsd : 0, floorStatus: eligible && !lowConfidence ? "priced" : estimateUsd > 0 ? "estimated-low" : "unavailable",
      valuationKind: eligible && !lowConfidence ? "username-estimate" : estimateUsd > 0 ? "username-estimate-low" : "unavailable", usernameValuationStatus: valuation.valuationStatus,
    ...
    return rows.map((asset) => {
      ...
      if (!valuation && existingBaselines?.size) {
        ...
        if (snapshot?.midpointUsd > 0) valuation = {
          ...
          confidenceScore: Math.min(0.3, 0.08 + Math.log1p(number(snapshot.evidenceCount)) * 0.04), confidenceBand: "low", valuationStatus: "indicative", portfolioEligible: estimatesEnabled,
