...
const PORTFOLIO_BANDS = new Set(["medium", "high"]);
...
function createUsernameRuntime(options = {}) {
  ...
  const firstImportTimeoutMs = Math.max(3_000, number(options.firstImportTimeoutMs) || 60_000);
  const estimatesEnabled = options.portfolioEstimatesEnabled ?? process.env.USERNAME_PORTFOLIO_ESTIMATES_ENABLED === "1";
  ...
  const firstImportEvidence = options.firstImportEvidence || (valuationReadModelUrl && valuationReadModelSecret
    ? createUsernameFirstImportEvidence({ logger }) : null);
    ...
    try {
      ...
      for (let index = 0; index < batchCount; index += 1) {
        const response = await fetchImpl(`${valuationReadModelUrl}/valuations/read`, {
          ...
          body: JSON.stringify({ assetKind: "username", assetKeys: addresses.slice(index * 500, (index + 1) * 500), assetNames: names.slice(index * 500, (index + 1) * 500) }),
  ...
  async function scoreProjectedAssets(assets) {
    if (!valuationReadModelUrl || !valuationReadModelSecret || !assets.length) return new Map();
    const targets = assets.slice(0, 100).flatMap((asset) => {
      try {
    ...
    try {
      ...
      const payload = await response.json();
      const events = (payload.records || []).map((row) => ({
      ...
      const records = targets.flatMap(({ asset, feature }) => {
        const estimate = estimateTelegramUsernameValue(feature, events, { nowMs: valuedAt.getTime(), learnedModel });
        ...
        return [{
          ...
          portfolioEligible: estimatesEnabled && estimate.ownSaleCount >= 2 && estimate.status === "estimated" && PORTFOLIO_BANDS.has(estimate.confidenceBand),
  ...
  async function enrichFirstImportAssets(assets) {
    ...
    const pending = assets.filter((asset) => {
      ...
      return key && now - attemptedAt >= 6 * 60 * 60 * 1000;
    });
  ...
  function needsProjectedScore(valuation) {
  ...
  function merge(asset, valuation) {
    ...
    const currentEstimator = !valuation.estimatorVersion || valuation.estimatorVersion === USERNAME_ESTIMATOR_VERSION;
    const eligible = estimatesEnabled && !stale && currentEstimator && valuation.portfolioEligible && PORTFOLIO_BANDS.has(String(valuation.confidenceBand || "").toLowerCase()) && estimateUsd > 0;
    return { ...asset,
      floorTon: 0, floorUsd: eligible ? estimateUsd : 0, floorStatus: eligible ? "priced" : estimateUsd > 0 ? "estimated-low" : "unavailable",
      valuationKind: eligible ? "username-estimate" : estimateUsd > 0 ? "username-estimate-low" : "unavailable", usernameValuationStatus: valuation.valuationStatus,
  ...
  async function valueAssets(assets) {
    ...
    let scoreRows = rows.filter((asset) => needsProjectedScore(valuations.get(address(asset.tokenAddress || asset.address))));
    if (scoreRows.length) {
      await enrichFirstImportAssets(scoreRows);
      const refreshed = await readProjectedValuations(scoreRows.map((asset) => asset.tokenAddress || asset.address), scoreRows.map((asset) => {
        try { return classifyTelegramUsername(asset.username || asset.name || asset.displayName).normalizedUsername; }
        catch { return ""; }
      }).filter(Boolean));
      ...
      scoreRows = rows.filter((asset) => needsProjectedScore(valuations.get(address(asset.tokenAddress || asset.address))));
    }
    if (scoreRows.length) {
      const scored = await scoreProjectedAssets(scoreRows);
      for (const [key, valuation] of scored) {
        valuations.set(key, valuation);
        cache.set(key, { value: valuation, expiresAt: Date.now() + 60_000 });
      }
    }
    let existingBaselines = null;
    if (configured() && rows.some((asset) => !valuations.has(address(asset.tokenAddress || asset.address)))) {
      try { existingBaselines = await baselines(); } catch (error) { logger.warn?.(`[username-estimator] baseline unavailable: ${error.message}`); }
    }
    return rows.map((asset) => {
      const key = address(asset.tokenAddress || asset.address);
      let valuation = valuations.get(key);
      if (!valuation && existingBaselines?.size) {
        const snapshot = baselineFor(asset, existingBaselines);
        if (snapshot?.midpointUsd > 0) valuation = {
          nftAddress: key, usernameNormalized: classifyTelegramUsername(asset.username || asset.name || asset.displayName).normalizedUsername,
          estimateUsd: snapshot.midpointUsd, rangeLowUsd: snapshot.rangeLowUsd, rangeHighUsd: snapshot.rangeHighUsd,
          confidenceScore: Math.min(0.3, 0.08 + Math.log1p(number(snapshot.evidenceCount)) * 0.04), confidenceBand: "low", valuationStatus: "indicative", portfolioEligible: false,
          evidenceCount: snapshot.evidenceCount, effectiveCompCount: snapshot.effectiveCompCount, ownSaleCount: 0, currentListingGram: 0, currentBidGram: 0,
          estimatorVersion: USERNAME_ESTIMATOR_VERSION, calibrationVersion: USERNAME_CALIBRATION_VERSION, explanation: { instant: true, provenance: "verified-sales-archetype-baseline" }, valuedAt: new Date(), staleAt: new Date(Date.now() + 300_000),
        };
      }
      return merge(asset, valuation);
    });
  }
  function enqueueAssets(assets) {
    return Promise.resolve().then(async () => {
      ...
      if (valuationReadModelUrl && valuationReadModelSecret && prepared.length) {
        const records = prepared.map(({ nftAddress, username }) => ({
  ...
  async function projectedUsernameValuation(username) {
    ...
    return projected.values().next().value || null;
  }
