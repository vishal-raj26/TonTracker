...
function createFragmentUsernameSource(options = {}) {
  ...
  const maximumDepth = Math.max(2, Math.min(8, Number(options.maximumDepth ?? process.env.USERNAME_FRAGMENT_PREFIX_DEPTH ?? 6)));
  ...
  // Fragment currently returns roughly 183 sold rows for a non-empty query,
  // despite returning 500 for the empty latest-sales query. Treat 180 as the
  // conservative dense-prefix boundary so capped partitions are expanded.
  const resultLimit = Math.max(50, Math.min(500, Number(options.resultLimit ?? process.env.USERNAME_FRAGMENT_RESULT_LIMIT ?? 180)));
