...
function trainUsernameLearnedModel(events = [], options = {}) {
  ...
  return {
    version: "username-learned-ridge-v3",
...
function predictUsernameLearnedModel(model, input) {
  if (!model || !["username-learned-ridge-v1", "username-learned-ridge-v2", "username-learned-ridge-v3"].includes(model.version) || !Array.isArray(model.coefficients)) return null;
  ...
  const patternAdjustment = patternWeight
    ...
    : 0;
  const center = dot(featureVector(classification), model.coefficients) + patternAdjustment;
