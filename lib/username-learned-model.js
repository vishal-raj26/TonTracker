...
function trainUsernameLearnedModel(events = [], options = {}) {
  ...
  return {
    version: "username-learned-ridge-v10",
...
function predictUsernameLearnedModel(model, input) {
  if (!model || model.version !== "username-learned-ridge-v10" || !Array.isArray(model.coefficients) || model.coefficients.length !== FEATURE_NAMES.length) return null;
  ...
  const effectivePremiumProbability = Number(cohort?.populationCount || cohort?.count || 0) >= 8
    ? Math.max(premiumProbability, Number(cohort.premiumRate || 0))
    : premiumProbability;
...
module.exports = { FEATURE_NAMES, featureVector, marketPatternKeys, predictUsernameLearnedModel, trainUsernameLearnedModel };
