  ...
  async function scoreProjectedDnsEntries(entries, gramUsdRate) {
    ...
    try {
      const response = await fetchImpl(`${valuationReadModelUrl}/identity/dns-evidence/read`, {
        ...
        body: JSON.stringify({ targets: targets.map((entry) => entry.request) }),
      ...
      const evidence = (payload.records || []).flatMap((row) => {
        try {
          const classification = classifyTonDns(row.normalized_name);
      ...
      const records = targets.flatMap((entry) => {
        const events = evidence.map((event) => ({
          ...event,
          semanticSimilarity: verifiedSemanticSimilarity(entry.target, event.classification),
        }));
        const estimate = estimateTonDnsValue(entry.target, events, {}, { now: valuedAt });
        if (estimate.status !== "estimated" || !(finiteNumber(estimate.estimateGram) > 0)) return [];
        const confidenceBand = String(estimate.confidenceBand || "low").toLowerCase();
        return [{
          assetKind: "dns",
          assetKey: entry.address,
          displayName: entry.target.normalizedDomain,
          ...
          confidenceBand,
          valuationStatus: estimate.status,
          portfolioEligible: portfolioEstimatesEnabled,
          evidenceCount: estimate.evidenceCount,
          effectiveCompCount: estimate.effectiveCompCount,
          ownSaleCount: estimate.ownSaleCount,
          ...
          explanation: {
            ...
            historicalUsd: true,
            route: entry.target.primaryRoute,
