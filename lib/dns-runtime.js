...
const PORTFOLIO_CONFIDENCE_BANDS = new Set(["low", "medium", "high"]);
...
function dnsKnowledgeOptions(name, knowledgeInput) {
  const knowledge = parseKnowledge(knowledgeInput);
  const label = String(name || "").toLocaleLowerCase("und").replace(/\.ton$/u, "");
  return {
    knowledge,
    dictionaryWords: knowledge.dictionaryMatch ? [label] : [],
    // An encyclopedia title match is not evidence that a TON DNS name has
    // market demand. Entity routing is reserved for a future curated market
    // identity source, never inferred from Wikipedia alone.
...
function createDnsRuntime(options = {}) {
  ...
  const firstImportTimeoutMs = Math.max(3_000, finiteNumber(options.firstImportTimeoutMs) || 60_000);
  ...
  const portfolioEstimatesEnabled = options.portfolioEstimatesEnabled
    ?? process.env.DNS_PORTFOLIO_ESTIMATES_ENABLED !== "0";
  ...
  function mergeValuation(asset, valuation, gramUsdRate = 0) {
    ...
    const staleAtMs = Date.parse(valuation.staleAt || "");
    const currentAndFresh = String(valuation.estimatorVersion || "") === DNS_ESTIMATOR_VERSION
      && Number.isFinite(staleAtMs)
      && staleAtMs > Date.now();
    const valuationStatus = String(valuation.valuationStatus || "").toLowerCase();
    const marketEstimated = ["estimated", "indicative"].includes(valuationStatus);
    ...
    return {
      ...
      valuationStale: !currentAndFresh,
  ...
  function verifiedSemanticSimilarity(target, comparable) {
    if (target.normalizedDomain === comparable.normalizedDomain) return 1;
    if (target.cryptoTerm && target.cryptoTerm === comparable.cryptoTerm) return 0.86;
    const targetTokens = new Set(target.compoundTokens || []);
    const comparableTokens = comparable.compoundTokens || [];
    const sharedTokens = comparableTokens.filter((token) => targetTokens.has(token));
    if (sharedTokens.length && target.primaryRoute === comparable.primaryRoute) {
      return Math.min(0.9, 0.72 + (0.08 * sharedTokens.length));
    }
    const targetKnowledge = parseKnowledge(target.knowledge);
    const comparableKnowledge = parseKnowledge(comparable.knowledge);
    const targetTerms = new Set((targetKnowledge.relatedTerms || []).map((term) => String(term).replace(/[^a-z0-9]/gi, "").toLowerCase()).filter(Boolean));
    const comparableTerms = new Set((comparableKnowledge.relatedTerms || []).map((term) => String(term).replace(/[^a-z0-9]/gi, "").toLowerCase()).filter(Boolean));
    const targetLabel = String(target.label || "").replace(/[^a-z0-9]/gi, "").toLowerCase();
    const comparableLabel = String(comparable.label || "").replace(/[^a-z0-9]/gi, "").toLowerCase();
    if (targetTerms.has(comparableLabel) || comparableTerms.has(targetLabel)) return 0.84;
    const sharedMeaning = [...targetTerms].filter((term) => comparableTerms.has(term)).length;
    const meaningUnion = new Set([...targetTerms, ...comparableTerms]).size;
    if (sharedMeaning >= 2 && meaningUnion && sharedMeaning / meaningUnion >= 0.12) return 0.76;
    const targetEntity = String(targetKnowledge.entityTitle || "").trim().toLowerCase();
    const comparableEntity = String(comparableKnowledge.entityTitle || "").trim().toLowerCase();
    if (targetEntity && targetEntity === comparableEntity) return 0.9;
    return null;
  }

  async function scoreProjectedDnsEntries(entries, gramUsdRate) {
    if (!valuationReadModelUrl || !valuationReadModelSecret || !entries.length || !(finiteNumber(gramUsdRate) > 0)) return new Map();
    const targets = entries.slice(0, 100).flatMap((entry) => {
      try {
        const target = classifyTonDns(domainOf(entry.asset));
        return [{ ...entry, target, request: {
          normalizedName: target.normalizedDomain,
          primaryRoute: target.primaryRoute,
          lengthBucket: dnsLengthBucket(target.characterLength),
        } }];
      } catch { return []; }
    });
    if (!targets.length) return new Map();
    try {
      const fetchEvidence = async (requests) => {
        const response = await fetchImpl(`${valuationReadModelUrl}/identity/dns-evidence/read`, {
          method: "POST",
          headers: { "content-type": "application/json", authorization: `Bearer ${valuationReadModelSecret}` },
          body: JSON.stringify({ targets: requests }), signal: AbortSignal.timeout(firstImportTimeoutMs),
        });
        if (!response.ok) throw new Error(`dns evidence ${response.status}`);
        return response.json();
      };
      let payload = await fetchEvidence(targets.map((entry) => entry.request));
      let knowledgeRows = [...(payload.records || []), ...(payload.knowledge || [])];
      let knowledgeByName = new Map(knowledgeRows.map((row) => [
        String(row.normalized_name || "").toLocaleLowerCase("und"), parseKnowledge(row.semantic_json),
      ]).filter(([name]) => name));
      const adjustedRequests = targets.map((entry) => {
        const knowledge = knowledgeByName.get(entry.target.normalizedDomain) || {};
        const classified = classifyTonDns(entry.target.normalizedDomain, dnsKnowledgeOptions(entry.target.normalizedDomain, knowledge));
        return { normalizedName: classified.normalizedDomain, primaryRoute: classified.primaryRoute, lengthBucket: dnsLengthBucket(classified.characterLength) };
      });
      const changedRoute = adjustedRequests.some((request, index) => request.primaryRoute !== targets[index].request.primaryRoute);
      if (changedRoute) {
        const adjusted = await fetchEvidence(adjustedRequests);
        const records = new Map([...(payload.records || []), ...(adjusted.records || [])].map((row) => [row.sale_id, row]));
        payload = { records: [...records.values()], knowledge: [...(payload.knowledge || []), ...(adjusted.knowledge || [])] };
        knowledgeRows = [...payload.records, ...payload.knowledge];
        knowledgeByName = new Map(knowledgeRows.map((row) => [
          String(row.normalized_name || "").toLocaleLowerCase("und"), parseKnowledge(row.semantic_json),
        ]).filter(([name]) => name));
      }
      const evidence = (payload.records || []).flatMap((row) => {
        try {
          const knowledge = knowledgeByName.get(String(row.normalized_name || "").toLocaleLowerCase("und")) || {};
          const classification = classifyTonDns(row.normalized_name, dnsKnowledgeOptions(row.normalized_name, knowledge));
          classification.knowledge = knowledge;
          let qualityFlags = row.quality_flags_json || [];
          if (typeof qualityFlags === "string") { try { qualityFlags = JSON.parse(qualityFlags); } catch { qualityFlags = []; } }
          return [{
            eventId: row.sale_id,
            ...
            paymentAsset: "GRAM",
            priceGram: finiteNumber(row.price_usd) / finiteNumber(gramUsdRate),
            priceUsd: finiteNumber(row.price_usd),
            finalized: true,
            verified: true,
            qualityScore: finiteNumber(row.reliability_score) || 1,
            qualityFlags,
          }];
        } catch { return []; }
      });
      const baselineValues = await loadBaselineSnapshot();
      const valuedAt = new Date();
      const staleAt = new Date(valuedAt.getTime() + 24 * 60 * 60 * 1000);
      const records = targets.flatMap((entry) => {
        const targetKnowledge = knowledgeByName.get(entry.target.normalizedDomain) || {};
        const target = classifyTonDns(entry.target.normalizedDomain, dnsKnowledgeOptions(entry.target.normalizedDomain, targetKnowledge));
        target.knowledge = targetKnowledge;
        const events = evidence.map((event) => ({
          ...event,
          semanticSimilarity: verifiedSemanticSimilarity(target, event.classification),
        }));
        const baseline = baselineFor(target, baselineValues);
        const marketBaseline = baseline?.midpointUsd > 0 && finiteNumber(gramUsdRate) > 0
          ? {
            ...baseline,
          ...
          : baseline;
        const estimate = estimateTonDnsValue(target, events, {}, { now: valuedAt, marketBaseline });
        if (!(finiteNumber(estimate.estimateGram) > 0)) return [];
        const confidenceBand = String(estimate.confidenceBand || "low").toLowerCase();
        return [{
          assetKind: "dns",
          assetKey: entry.address,
          displayName: target.normalizedDomain,
          estimateUsd: estimate.estimateGram * finiteNumber(gramUsdRate),
          rangeLowUsd: estimate.rangeLowGram * finiteNumber(gramUsdRate),
          rangeHighUsd: estimate.rangeHighGram * finiteNumber(gramUsdRate),
          confidenceScore: estimate.confidenceScore,
          confidenceBand,
          valuationStatus: estimate.status,
          portfolioEligible: portfolioEstimatesEnabled && estimate.status === "estimated",
          evidenceCount: estimate.evidenceCount,
          effectiveCompCount: estimate.effectiveCompCount,
          ownSaleCount: estimate.ownSaleCount,
          currentListingGram: entry.asset.valuationKind === "active-listing" ? finiteNumber(entry.asset.floorTon) : finiteNumber(entry.asset.listingGram),
          currentBidGram: 0,
          ...
          explanation: {
            provenance: "first-import-completed-dns-sales",
            historicalUsd: true,
            route: target.primaryRoute,
            comparableNames: estimate.comparables.slice(0, 8).map((row) => row.domain),
          },
        }];
      });
      if (records.length) {
        ...
        if (!write.ok) throw new Error(`dns valuation cache ${write.status}`);
      }
      return new Map(records.map((record) => [canonicalTonAddress(record.assetKey), record]));
    } catch (error) {
      logger.warn?.(`[dns-estimator] first-import scoring unavailable: ${error.message}`);
      return new Map();
    }
  }
  ...
  async function valueAssets(assets, gramUsdRate = 0, addressOf = (asset) => asset.tokenAddress || asset.address) {
    ...
    let missing = keyed.filter((entry) => entry.address && needsInstantValuation(valuations.get(entry.address)));
    if (missing.length) {
      const scored = await scoreProjectedDnsEntries(missing, gramUsdRate);
      for (const [address, valuation] of scored) {
        valuations.set(address, valuation);
        valuationCache.set(address, { value: valuation, expiresAt: Date.now() + cacheTtlMs });
      }
      missing = keyed.filter((entry) => entry.address && needsInstantValuation(valuations.get(entry.address)));
    }
    if (missing.length) {
  ...
  function enqueueAssets(assets, options = {}) {
    ...
    return Promise.resolve().then(async () => {
      ...
      if (valuationReadModelUrl && valuationReadModelSecret && prepared.length) {
        ...
        const assetRecords = prepared.map(({ nftAddress, domainNormalized }) => {
          const feature = classifyTonDns(domainNormalized);
          return {
            assetKind: "dns", assetKey: nftAddress, normalizedName: domainNormalized, displayName: domainNormalized,
            primaryRoute: feature.primaryRoute, lengthBucket: dnsLengthBucket(feature.characterLength),
        ...
        if (!assetResponse.ok) throw new Error(`dns identity assets ${assetResponse.status}`);
        const aliasRecords = prepared.map(({ nftAddress, domainNormalized }) => ({
