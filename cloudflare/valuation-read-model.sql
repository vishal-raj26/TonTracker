-- Compact, portfolio-facing projection only. Raw evidence stays outside D1.
...
CREATE INDEX IF NOT EXISTS valuation_records_lookup_idx
  ON valuation_records(asset_kind, stale_at);
...
-- Rebuildable, normalized evidence used by the DNS and Username estimators.
-- Raw source responses never belong in this database.
...
CREATE UNIQUE INDEX IF NOT EXISTS identity_assets_name_idx
  ON identity_assets(asset_kind, normalized_name);
CREATE INDEX IF NOT EXISTS identity_assets_archetype_idx
  ON identity_assets(asset_kind, primary_route, length_bucket, script, scarcity_class);

-- Wallet imports provide real NFT item addresses. Market discovery can start
-- with a synthetic catalog key, so preserve the address-to-name link without
-- duplicating raw market data or changing the canonical valuation key.
...
CREATE INDEX IF NOT EXISTS identity_sales_asset_time_idx
  ON identity_sales(asset_kind, asset_key, sold_at DESC);
CREATE INDEX IF NOT EXISTS identity_sales_name_time_idx
  ON identity_sales(asset_kind, normalized_name, sold_at DESC);
CREATE INDEX IF NOT EXISTS identity_sales_comparable_idx
  ON identity_sales(asset_kind, primary_route, length_bucket, script, scarcity_class, sold_at DESC);
...
CREATE INDEX IF NOT EXISTS identity_current_market_stale_idx
  ON identity_current_market(asset_kind, stale_at);
...
CREATE INDEX IF NOT EXISTS identity_archetype_baselines_ready_idx
  ON identity_archetype_baselines(asset_kind, estimator_version, stale_at);
...
-- Counters make quota decisions constant-time. A maintenance reconciliation
-- periodically corrects them from actual table counts.
